...
def eval_to_score_file(score_file, cm_key_file):
    submission_scores = pl.read_csv(
        ...
        new_columns=['file', 'score']
    )
    
    cm_data = pl.read_csv(
        cm_key_file,
        separator=',',
        has_header=True, # file,speaker,label
        truncate_ragged_lines=True
    )
    
    if submission_scores.height != cm_data.height:
        print(f'CHECK: submission has {submission_scores.height} of {cm_data.height} expected trials.')
        exit(1)

    if submission_scores.width != 2:
        print(f'CHECK: submission has {submission_scores.width} columns, expected 2. Check for blank spaces.')
        exit(1)
    
    # 根据 file 名拼接，结果为 'file', 'score', 'speaker', 'label'
    ...
    bona_scores = cm_scores.filter(pl.col('label') == 'bona-fide')['score'].to_numpy()
...
def main():
    parser = argparse.ArgumentParser(description='Get In-the-Wild EER scores using official script')
    ...
    parser.add_argument("--dataset", type=str, help="dataset name", required=True, choices=['itw'])
    ...
    if args.dataset not in ['itw']:
        raise ValueError("For this script, --dataset must be 'itw'")

    cm_key_file = "../Datasets/in_the_wild/meta.csv"
